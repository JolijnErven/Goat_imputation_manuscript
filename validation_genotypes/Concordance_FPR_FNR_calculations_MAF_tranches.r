...
# Define Minor Allele Frequency (MAF) tranches
a=c(0,0.01,0.02,0.05,0.1,0.2)
b=c(0.01,0.02,0.05,0.1,0.2,0.5)

# Iterate through each MAF tranch
for (i in seq_along(a)) {
   # Define MAF range as a string and extract MAF lower and upper bounds
   MAF <- paste0(a[i],"-",b[i])
   MAF1 <- a[i]
   MAF2 <- b[i]
   ...
   # Filter variants within the current MAF tranch
   val.GP.05<-val.GP[((val.GP$RAF >= MAF1) & (val.GP$RAF < MAF2)) | ( ((1-val.GP$RAF) >= MAF1) & ((1- val.GP$RAF) < MAF2)) ,]
...
write.csv(x = df,file=paste0(args[1],"_MAF_bins_concordance.csv"), quote = F, row.names = F)
write.csv(x = df2,file=paste0(args[1],"_MAF_bins_falseHetRate.csv"), quote = F, row.names = F)
write.csv(x = df3,file=paste0(args[1],"_MAF_bins_concordance_type.csv"), quote = F, row.names = F)
write.csv(x = df4,file=paste0(args[1],"_MAF_bins_falseHetRate_type.csv"), quote = F, row.names = F)
write.csv(x = df.het,file=paste0(args[1],"_MAF_bins_het_concordance.csv"), quote = F, row.names = F)
write.csv(x = df3.het,file=paste0(args[1],"_MAF_bins_het_concordance_type.csv"), quote = F, row.names = F)
write.csv(x = df5,file=paste0(args[1],"_MAF_bins_concordance_hom.csv"), quote = F, row.names = F)
write.csv(x = df6,file=paste0(args[1],"_MAF_bins_concordance_hom_type.csv"), quote = F, row.names = F)
write.csv(x = df.count,file=paste0(args[1],"_MAF_bins_count.csv"), quote = F, row.names = F)
write.csv(x = df.count.type,file=paste0(args[1],"_MAF_bins_count_type.csv"), quote = F, row.names = F)
write.csv(x = df7,file=paste0(args[1],"_MAF_bins_concordance_hom_alt.csv"), quote = F, row.names = F)
write.csv(x = df8,file=paste0(args[1],"_MAF_bins_concordance_hom_alt_type.csv"), quote = F, row.names = F)
write.csv(x = df10,file=paste0(args[1],"_MAF_bins_concordance_hom_all.csv"), quote = F, row.names = F)
write.csv(x = df9,file=paste0(args[1],"_MAF_bins_concordance_hom_all_type.csv"), quote = F, row.names = F)
write.csv(x = df.FN1,file=paste0(args[1],"_MAF_bins_false_negHetRate.csv"), quote = F, row.names = F)
write.csv(x = df.FN2,file=paste0(args[1],"_MAF_bins_false_negHetRate_type.csv"), quote = F, row.names = F)
write.csv(x = df.data,file=paste0(args[1],"_MAF_bins_genotype_information.csv"), quote = F, row.names = F)
