...
ls *chr9*_concordance_500kb_100kb.bed | cut -f 2-50 -d "_" > concat.txt
...
while read i; do
    ...
    # Get lowest 1 percentile of concordance regions
    sort -k5 -n Merged_"$i".bed | head -${perc} | sort -k 1,1 -k 2,2 -n | sed 's/ /\t/g' > Merged_"$i"_99.9_perc.bed
...
ls Merged_*concordance_500kb_100kb_99.9_perc_region.bed | cut -f 3-50 -d "_" > get_majority.txt
...
# Get sites that have low concordance in majority of the individuals (majority__0.5X_GP0.99_validationMAF_0.05_Het_homoalt_concordance_500kb_100kb_99.9_perc_region.bed file)
